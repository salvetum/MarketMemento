...
const copy = {
  tr: {
    ...
    privacyTitle: 'Gizlilik özeti', privacyData: 'Yüklediğin CSV dosyaları yalnızca bu tarayıcı sekmesinde işlenir. Dosya içeriği MarketMemento sunucularına gönderilmez.', privacyStorage: 'Ayarlar, dil, tema ve analiz önbelleği tarayıcının yerel depolamasında tutulabilir. Tarayıcı verilerini temizleyerek bunları silebilirsin.', privacyThirdParty: 'Canlı kur dönüşümü için yalnızca para birimi kodları Frankfurter API’ye gönderilebilir. Steam Market fiyat istekleri yalnızca canlı fiyat yenilemesini seçtiğinde yapılır.', privacyRights: 'İstediğin zaman dosyayı kaldırabilir, yeni dosya yükleyebilir veya tarayıcı depolamasını temizleyebilirsin. Bu proje kişisel bir araçtır; yasal danışmanlık sunmaz.',
  ...
  en: {
    ...
    privacyTitle: 'Privacy summary', privacyData: 'Uploaded CSV files are processed in this browser. File contents are not sent to MarketMemento servers.', privacyStorage: 'Settings, language, theme, and analysis cache may be stored in browser storage. Clearing browser data removes them.', privacyThirdParty: 'Only currency codes may be sent to the Frankfurter API for live conversion rates. Steam Market requests are made only when you request a live price refresh.', privacyRights: 'You can remove the file, load a new one, or clear browser storage at any time. This is a personal tool and does not provide legal advice.',
