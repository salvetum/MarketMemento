...
function Modal({ title, onClose, children, closeLabel }) {
  return <div className="react-modal-backdrop" role="presentation" onMouseDown={event => {
    ...
    <div className="modal-header border-0"><h2 id="react-modal-title" className="modal-title fs-5">{title}</h2>
      <button type="button" className="btn-close" onClick={onClose} aria-label={closeLabel} /></div>
    <div className="modal-body pt-0">{children}</div>
  </div></div>;
}

function ExportButton({ tableId, rows, columns, format }) {
  const exportRows = () => {
  ...
  return <button className="export-button" type="button" onClick={exportRows} title={format.toUpperCase()} aria-label={`Export ${format.toUpperCase()}`}>
...
function DataTable({ id, title, rows, columns, t, pageSize = 25 }) {
  ...
  return <section className="table-section tab-panel active">
    <div className="section-heading"><h2>{title}</h2><div className="table-tools"><input className="table-search" value={query} placeholder={t('search')} onChange={event => { setQuery(event.target.value); setPage(1); }} />
      <ExportButton tableId={id} rows={matching} columns={columns} format="csv" /><ExportButton tableId={id} rows={matching} columns={columns} format="json" /></div></div>
...
function Heatmap({ analysis, t }) {
  ...
  return <div className="heatmap-wrap"><div className="heatmap-grid" style={{ '--heatmap-columns': labels.length }}>{analysis.activity.map((row, day) => row.map((value, index) => {
    ...
    return <span key={`${day}-${index}`} title={`${value} ${t('transactions')}`} className="heatmap-cell" data-level={level} aria-label={`${value} ${t('transactions')}`} />;
  }))}</div><div className="heatmap-axis">{labels.map(label => <small key={label}>{label}</small>)}</div></div>;
}
...
function ChartGrid({ analysis, t, formatMoney }) {
  ...
  return <div className="chart-grid tab-panel active">
    ...
    <div className="chart-card" id="roi-profit-chart"><NivoChart options={{ chart: { type: 'bar' }, title: { text: t('profit') }, xaxis: { categories: analysis.profitAnalysis.filter(item => item.net > 0).slice(0, 6).map(item => item.name), }, series: [{ data: analysis.profitAnalysis.filter(item => item.net > 0).slice(0, 6).map(item => item.net) }], colors: ['#57e6a5'] }} /></div>
    <div className="chart-card" id="roi-loss-chart"><NivoChart options={{ chart: { type: 'bar' }, title: { text: t('profit') }, xaxis: { categories: analysis.profitAnalysis.filter(item => item.net < 0).slice(0, 6).map(item => item.name), }, series: [{ data: analysis.profitAnalysis.filter(item => item.net < 0).slice(0, 6).map(item => item.net) }], colors: ['#ff718d'] }} /></div>
    <div className="chart-card chart-wide" id="game-comparison-chart"><NivoChart options={{ chart: { type: 'bar' }, title: { text: t('comparison') }, xaxis: { categories: analysis.gameComparison.map(item => item.name) }, series: [{ name: t('comparisonSpent'), data: analysis.gameComparison.map(item => item.spent) }, { name: t('comparisonEarned'), data: analysis.gameComparison.map(item => item.earned) }, { name: t('comparisonCashflow'), data: analysis.gameComparison.map(item => item.cashflow) }], colors: ['#a684ff', '#67c1f5', '#57e6a5'] }} /></div>
  </div>;
}
...
function App() {
  ...
  return <><div className="ambient" aria-hidden="true"><span className="orb orb-blue" /><span className="orb orb-violet" /><span className="orb orb-green" /><span className="grid-overlay" /></div>
    <main className="site-shell">
      ...
      </section> : <section id="dashboard" className={`dashboard ${exporting ? 'exporting' : ''}`}><div className="dashboard-header glass-panel"><div><span className="eyebrow"><i className="bi bi-activity" /> {t('dashboard')}</span><h1>{t('dashboard')}</h1><p>{demo ? `${t('demoData')} ·` : fileNames.join(', ')} {filtered.length.toLocaleString(lang)} {t('records')}</p></div><div className="dashboard-tools"><div className="dashboard-actions"><label className="secondary-button action-label"><input type="file" accept=".csv,text/csv" multiple hidden onChange={event => importFiles(event.target.files)} /><i className="bi bi-file-earmark-plus" /> {t('add')}</label>      <label className="quick-currency"><i className="bi bi-currency-exchange" /><select value={currency} onChange={event => updateCurrency(event.target.value)}>{currencyOptions.map(value => <option key={value}>{value}</option>)}</select></label><button className="secondary-button" type="button" onClick={() => setModal('settings')}><i className="bi bi-sliders" /> {t('settings')}</button>{installPrompt ? <button type="button" className="secondary-button" onClick={async () => { await installPrompt.prompt(); setInstallPrompt(null); }}><i className="bi bi-download" /> {t('install')}</button> : null}<button type="button" className="secondary-button" onClick={() => doExport('png')} disabled={exporting}><i className="bi bi-image" /> PNG</button><button type="button" className="secondary-button" onClick={() => doExport('pdf')} disabled={exporting}><i className="bi bi-file-earmark-pdf" /> PDF</button><button type="button" className="secondary-button" onClick={reset}><i className="bi bi-arrow-counterclockwise" /> {t('newFile')}</button></div><small className="export-hint">{t('exportHint')}</small></div></div>
        {status ? <div className="dashboard-status error" role="status">{status}</div> : null}<div id="import-report" className="import-report glass-panel" hidden={!invalidDateCount && !duplicatesRemoved}>{invalidDateCount} {t('invalid')} · {duplicatesRemoved} {t('duplicates')}</div><div className="filter-bar glass-panel"><label><span>{t('game')}</span><select className="form-select" value={game} onChange={event => setGame(event.target.value)}><option value="">{t('all')}</option>{games.map(value => <option key={value}>{value}</option>)}</select></label><label><span>{t('item')}</span><input className="form-control" value={item} placeholder={t('item')} onChange={event => setItem(event.target.value)} /></label><label><span>{t('type')}</span><select className="form-select" value={type} onChange={event => setType(event.target.value)}><option value="">{t('allTypes')}</option><option value="purchase">{t('purchaseType')}</option><option value="sale">{t('saleType')}</option></select></label><label><span>{t('sourceCurrencyLabel')}</span><select className="form-select" value={sourceFilter} onChange={event => setSourceFilter(event.target.value)}><option value="">{t('allTypes')}</option>{currencies.map(value => <option key={value}>{value}</option>)}</select></label><label><span>{t('preset')}</span><select className="form-select" value={datePreset} onChange={event => setDatePreset(event.target.value)}><option value="">{t('allTime')}</option><option value="30">{t('last30')}</option><option value="90">{t('last90')}</option><option value="ytd">{t('yearToDate')}</option></select></label><label><span>{t('from')}</span><input type="month" className="form-control" min={months[0]} max={months.at(-1)} value={from} disabled={Boolean(datePreset)} onChange={event => setFrom(event.target.value)} /></label><label><span>{t('to')}</span><input type="month" className="form-control" min={months[0]} max={months.at(-1)} value={to} disabled={Boolean(datePreset)} onChange={event => setTo(event.target.value)} /></label><button type="button" className="secondary-button" onClick={() => { setGame(''); setItem(''); setFrom(''); setTo(''); setType(''); setSourceFilter(''); setDatePreset(''); }}>{t('clear')}</button></div>
        <div className="analytics-panel glass-panel">{analysisLoading ? <div className="dashboard-status" role="status">{t('reading')}</div> : null}{analysis?.inventory?.length ? <div className="inventory-live-tools"><button type="button" className="secondary-button" onClick={refreshMarketPrices} disabled={pricesLoading}><i className="bi bi-cloud-download" /> {t('refreshPrices')}</button>{pricesStatus ? <span role="status">{pricesStatus}</span> : null}</div> : null}<div className="summary-grid" id="summary-cards">{analysis ? [{ label: t('realised'), value: formatMoney(analysis.summary.realisedProfit), icon: 'bi-graph-up-arrow', tone: analysis.summary.realisedProfit >= 0 ? 'positive' : 'negative' }, { label: t('sold'), value: formatMoney(analysis.summary.totalSold), icon: 'bi-arrow-up-right-circle' }, { label: t('bought'), value: formatMoney(analysis.summary.totalPurchased), icon: 'bi-arrow-down-left-circle' }, { label: t('fees'), value: formatMoney(analysis.summary.totalFees), icon: 'bi-percent' }, { label: t('inventory'), value: analysis.summary.remainingQuantity.toLocaleString(lang), icon: 'bi-box-seam' }, { label: t('inventoryCost'), value: formatMoney(analysis.summary.remainingCost), icon: 'bi-wallet2' }, { label: t('inventoryValue'), value: formatMoney(analysis.summary.remainingValue), icon: 'bi-bar-chart-line' }, { label: t('unrealised'), value: formatMoney(analysis.summary.unrealisedProfit), icon: 'bi-graph-up', tone: analysis.summary.unrealisedProfit >= 0 ? 'positive' : 'negative' }, { label: t('unrealisedRoi'), value: `${analysis.summary.unrealisedRoi.toFixed(2)}%`, icon: 'bi-percent' }, { label: t('holdingDays'), value: `${analysis.summary.averageHoldingDays.toFixed(0)} ${lang === 'tr' ? 'gün' : 'days'}`, icon: 'bi-hourglass-split' }, { label: t('transactions'), value: analysis.summary.transactions.toLocaleString(lang), icon: 'bi-receipt' }].map(card => <article className={`stat-card ${card.tone || ''}`} key={card.label}><div className="stat-top"><span className="stat-label">{card.label}</span><span className="stat-icon"><i className={`bi ${card.icon}`} /></span></div><strong className="stat-value">{card.value}</strong></article>) : null}</div>
          ...
          {analysis && tab === 'activity-panel' ? <div className="activity-grid tab-panel active"><div className="activity-card"><div className="section-heading"><h2>{t('heatmap')}</h2></div><div id="activity-heatmap"><Heatmap analysis={analysis} t={t} /></div></div><div className="activity-card" id="year-comparison-chart"><NivoChart options={{ chart: { type: 'bar' }, title: { text: t('year') }, xaxis: { categories: Array.from({ length: 12 }, (_, index) => String(index + 1)) }, series: analysis.yearSeries.map(series => ({ name: series.name, data: series.data })) }} /></div></div> : null}
        </div>
      </section>}
      <footer><span>MarketMemento</span><span>{t('privacy')}</span></footer>
    </main>
    {pending ? <Modal title={t('preview')} closeLabel={t('close')} onClose={() => setPending(null)}><p>{pending.files.length} {t('files')} · {pending.deduped.rows.length.toLocaleString(lang)} {t('accepted')} · {pending.deduped.duplicates} {t('duplicates')} · {pending.invalidDateCount} {t('invalid')} · {pending.failed} {t('failed')}</p>{pending.failedFiles.length ? <div className="upload-status error" role="alert"><strong>{t('failedFiles')}</strong><ul>{pending.failedFiles.map(file => <li key={file}>{file}</li>)}</ul></div> : <p className="text-muted">{t('noFailed')}</p>}<div className="import-preview-actions"><button className="secondary-button" type="button" onClick={() => setPending(null)}>{t('cancel')}</button><button className="primary-button" type="button" onClick={confirmImport}>{t('confirm')}</button></div></Modal> : null}
    {modal === 'info' ? <Modal title={t('info')} closeLabel={t('close')} onClose={() => setModal(null)}><p>{t('privacy')}</p><p>{lang === 'tr' ? 'CSV dosyaları tarayıcından çıkmaz. Analiz yerel olarak yapılır ve IndexedDB içinde saklanabilir.' : 'CSV files never leave your browser. Analysis is performed locally and may be saved in IndexedDB.'}</p><p>{lang === 'tr' ? 'MarketMemento, Valve, Steam veya Steam Inventory Helper ile bağlantılı değildir.' : 'MarketMemento is not affiliated with Valve, Steam or Steam Inventory Helper.'}</p></Modal> : null}
