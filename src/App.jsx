...
function ChartGrid({ analysis, t, formatMoney }) {
  ...
  return <div className="chart-grid tab-panel active">
    ...
    <div className="chart-card" id="roi-profit-chart"><NivoChart options={{ chart: { type: 'bar' }, title: { text: t('profit') }, xaxis: { categories: analysis.profitAnalysis.filter(item => item.net > 0).slice(0, 6).map(item => item.name), }, series: [{ name: t('net'), data: analysis.profitAnalysis.filter(item => item.net > 0).slice(0, 6).map(item => item.net) }], colors: ['#57e6a5'] }} /></div>
    <div className="chart-card" id="roi-loss-chart"><NivoChart options={{ chart: { type: 'bar' }, title: { text: t('profit') }, xaxis: { categories: analysis.profitAnalysis.filter(item => item.net < 0).slice(0, 6).map(item => item.name), }, series: [{ name: t('net'), data: analysis.profitAnalysis.filter(item => item.net < 0).slice(0, 6).map(item => item.net) }], colors: ['#ff718d'] }} /></div>
    <div className="chart-card chart-wide" id="game-comparison-chart"><NivoChart options={{ chart: { type: 'bar' }, plotOptions: { bar: { horizontal: true } }, title: { text: t('comparison') }, xaxis: { categories: analysis.gameComparison.map(item => item.name) }, series: [{ name: t('comparisonSpent'), data: analysis.gameComparison.map(item => item.spent) }, { name: t('comparisonEarned'), data: analysis.gameComparison.map(item => item.earned) }, { name: t('comparisonCashflow'), data: analysis.gameComparison.map(item => item.cashflow) }], colors: ['#a684ff', '#67c1f5', '#57e6a5'] }} /></div>
  </div>;
}
